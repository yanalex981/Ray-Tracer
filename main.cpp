...
int main()
{
	...
	for (size_t x = 0; x < window_width; ++x)
	{
		for (size_t y = 0; y < window_height; ++y)
		{
			...
			double diffuse = bulb.intensity * hit.obj->m_material.k_diffuse * std::max(0.0, dot(s, hit.normal));

			vec3d ref = -s + hit.normal * 2 * dot(s, hit.normal);
			vec3d v = norm(eye - hit.world_pt);

			double specular = bulb.intensity * hit.obj->m_material.k_specular * std::pow(std::max(0.0, dot(ref, v)), hit.obj->m_material.fallout);

			auto r = static_cast<uint8_t>(clamp(std::round(diffuse * hit.obj->m_material.color.x() + ambient + specular), 255.0, 0.0));
			auto g = static_cast<uint8_t>(clamp(std::round(diffuse * hit.obj->m_material.color.y() + ambient + specular), 255.0, 0.0));
			auto b = static_cast<uint8_t>(clamp(std::round(diffuse * hit.obj->m_material.color.z() + ambient + specular), 255.0, 0.0));
